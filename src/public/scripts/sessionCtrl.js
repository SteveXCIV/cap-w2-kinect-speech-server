(function() {
    angular.module('ngCapstone').controller('sessionCtrl', function($scope, sessionFactory, $stateParams) {
        ...
        function rangePlotGenerator (trialData) {
            ...
            var lefthand = [];
            var spinemid = [];
            var righthandcolors = []; var lefthandcolors = [];
                            ...
                            for (var j = 0; j < 15; j++) {
                                angular.forEach(bodysnapshots[s].Joints[j], function(value, key) {
                                    if (key === "JointType" && value === "HandRight") {
                                        righthand.push(
                                        {
                                            x: parseFloat(bodysnapshots[s].Joints[j].X.toFixed(4)),
                                            y: parseFloat(bodysnapshots[s].Joints[j].Y.toFixed(4)),
                                            r: parseFloat(bodysnapshots[s].Joints[j].Z.toFixed(4)) * 10
                                        }
                                        );
                                    };
                                    ...
                                    righthandcolors.push('#45b7cd');
                                    lefthandcolors.push('#ff8533');
                                });
                            };
                        };
                    };
            ...
            return {right: righthand, left: lefthand, spinemid: spinemid, options: rangeoptions, rightcolors: righthandcolors, leftcolors: lefthandcolors};
        };
